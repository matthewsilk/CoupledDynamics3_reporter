...
A_concern_y<-tparams2$start
B_concern_y<-tparams2$start
...
for(t in 2:time){
  ...
  if(t==2){
    ...
    current<-concern_timestepN(pop_info=pop_info,net_b=info_mat,net_d=dis_mat,belief=start[[1]],concern=start[[2]],inf=inf,lA_ex,lB_ex,l_conc,l_conc_o,l_inf,l_inf_o,l_hea,l_hea_o,p_inf=p_inf,incs=rep(0,pop_info$pop))
